...
module Isomorfeus
  module Data
    module Handler
      class File < LucidHandler::Base
        # responsible for loading:
        # LucidData::File

        def process_request(response_agent)
          # promise_send_path('Isomorfeus::Data::Handler::File', self.to_s, action, props_hash)
          ...
          response_agent.error = { error: "Isomorfeus::Data::Handler::File: #{e.message}\n#{e.backtrace.join("\n")}" }
        end

        def process_create(response_agent, type_class, type_class_name)
          # 'Isomorfeus::Data::Handler::File', self.name, :create, data_hash
        ...
        def process_execute(response_agent, type_class, type_class_name)
          # 'Isomorfeus::Data::Handler::File', self.name, :execute, props_json
        ...
        def process_save(response_agent, type_class, type_class_name)
          # 'Isomorfeus::Data::Handler::File', self.name, :save, data_hash
